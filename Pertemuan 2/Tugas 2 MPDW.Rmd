...
## input data mengenai total CO2 emissions di indonesia dari tahun 1889 sampai 2014 (ada 126 observasi)
```{r}
df <- read.csv("dataPertemuan2.csv")
df
```

## Eksplorasi data
## plot time series data dari tahun 1889 sampai 2014 mengenai CO2 emissions dan didapatkan polanya tidak beraturan yaitu stasioner lalu kemudian ada pola tren naik
```{r}
data.ts <- ts(df$Total)
...
## plot pada DMA menunjukkan bahwa DMA hanya bagus saat memprediksi data latih sedangkan buruk dalam memprediksi data uji dengan hasil prediksi yang masih jauh dengan nilai aktual
```{r}
data_ts = ts(df$Total)
...
## Persamaan regresi yang dihasilkan adalah Total = 630,28 * sqrt(Solid.Fuel) -3967,59 dan memiliki nilai R-squared yang baik yaitu 92 % dan nilai fhitung yang besar sehingga peubah sqrt(Solid.Fuel) berpengaruh secara signifikan.
```{r}
#Pembuatan Model Regresi
#model regresi
model<- lm(df$Total~sqrt(df$Solid.Fuel))
summary(model)
```
...
## sisaan tidak menyebar normal menurut kolmogorov-smirnov dengan alfa 5%
